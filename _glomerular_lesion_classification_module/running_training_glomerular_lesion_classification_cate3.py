...
# -*- coding: utf-8 -*-
"""
Created on Tue Apr 12 14:45:45 2022
...
path = 'Glomerulus.Category3/'
Tpth = os.path.join(path,'Train')
Vpth = os.path.join(path,'Valid')

os.makedirs('modelbase/models_cate3/', exist_ok=True)
batchsize = 96
maxepochs = 300
lr = 1e-4

writer = SummaryWriter('Log/models_cate3/')
...
for epoch in range(1,maxepochs + 1):
    ...
    for imgs,lbls in tqdm(dataloaders['valid']):
        ...
        valid_corrects += torch.sum(preds == lbls.data)
    valid_acc  = valid_corrects.double() /  datasets_sizes['valid']
    valid_f1_IgA,valid_f1_MN,valid_f1_MCD = f1_score(real_vd,pred_vd,average=None)
    valid_f1_all = (valid_f1_IgA + valid_f1_MN + valid_f1_MCD)/3
    print('[{}] >>> f1_all:{:.4f} f1_IgAN:{:.4f} f1_MN:{:.4f} f1_MCD:{:.4f} acc:{:.4f}'.format(
        'Valid', valid_f1_all, valid_f1_IgA, valid_f1_MN, valid_f1_MCD, valid_acc
    ))
    writer.add_scalar('valid/acc',    valid_acc,    epoch)
    writer.add_scalar('valid/f1_all', valid_f1_all, epoch)
    writer.add_scalar('valid/f1_IgAN',valid_f1_IgA, epoch)
    writer.add_scalar('valid/f1_MN',  valid_f1_MN,  epoch)
    writer.add_scalar('valid/f1_MCD', valid_f1_MCD, epoch)

    if  valid_f1_all >= best_score_valid:
        best_score_valid = valid_f1_all
        torch.save(model,'modelbase/models_cate3/best_model.pkl')

torch.save(model,'modelbase/models_cate3/last_model.pkl')
writer.close()
